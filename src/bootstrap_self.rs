...
impl<'a> BootstrapSelf<'a> {
    ...
    pub fn run(&self) -> Result<BootstrapOutcome> {
        ...
        let repo_props = json!({
            ...
            "phases": phases.clone(),
            "bootstrap_source": "bootstrap-self",
            "captured_at": Utc::now().to_rfc3339(),
        });

        let repo_node_id = self.persistence.insert_graph_node(
            self.session_id,
            NodeType::Entity,
            "Repository",
            &repo_props,
        ...
        for component in &components {
            ...
            self.persistence.insert_graph_edge(
                ...
                EdgeType::PartOf,
                Some("component_of"),
                Some(&json!({"bootstrap_source": "bootstrap-self"})),
                0.95,
            )?;
            edges_created += 1;
        }
        ...
        for document in &documents {
            ...
            let doc_node_id = self.persistence.insert_graph_node(
                self.session_id,
                NodeType::Fact,
                "RepositoryDocument",
                &doc_props,
        ...
        if !manifest.is_empty() {
            ...
            let manifest_node_id = self.persistence.insert_graph_node(
                self.session_id,
                NodeType::Concept,
                "CargoManifest",
                &manifest_props,
    ...
    fn collect_repo_metadata(&self) -> Result<RepoMetadata> {
        let manifest_path = self.repo_root.join("Cargo.toml");
    ...
    fn collect_components(&self) -> Result<Vec<RepoComponent>> {
        let mut components = Vec::new();
        let entries = fs::read_dir(&self.repo_root)
            .with_context(|| format!("reading {}", self.repo_root.display()))?;
        ...
        for entry in entries {
            ...
            if entry.file_type()?.is_dir() {
                let stats = self.summarize_directory(&path)?;
                let relative_path = to_relative_string(&path, &self.repo_root);
    ...
    fn summarize_directory(&self, path: &Path) -> Result<ComponentStats> {
        ...
        for entry in WalkDir::new(path).min_depth(1).into_iter() {
            ...
            stats.total_files += 1;
            let rel = to_relative_string(entry.path(), &self.repo_root);
    ...
    fn collect_documents(&self) -> Vec<DocumentDigest> {
        let mut documents = Vec::new();
        let mut push_if_present = |relative: &str| {
            let path = self.repo_root.join(relative);
            if path.is_file() {
                if let Some(digest) = digest_document(&path, &self.repo_root) {
        ...
        if docs_dir.is_dir() {
                ...
                .filter_map(Result::ok)
            {
                ...
                {
                    if let Some(digest) = digest_document(entry.path(), &self.repo_root) {
                        documents.push(digest);
                    }
                }
        ...
        let specs_dir = self.repo_root.join("specs");
