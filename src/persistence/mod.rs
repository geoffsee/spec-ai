...
use crate::types::{Message, MessageRole, MemoryVector, PolicyEntry};
...
impl Persistence {
    ...
    pub fn new<P: AsRef<Path>>(db_path: P) -> Result<Self> {
        ...
        if let Some(dir) = db_path.parent() { std::fs::create_dir_all(dir).context("creating DB directory")?; }
    ...
    pub fn conn(&self) -> Result<Connection> { Connection::open(&self.db_path).context("opening DuckDB connection") }

    // ---------- Messages ----------

    pub fn insert_message(&self, session_id: &str, role: MessageRole, content: &str) -> Result<i64> {
        let conn = self.conn()?;
        let mut stmt = conn.prepare("INSERT INTO messages (session_id, role, content) VALUES (?, ?, ?) RETURNING id")?;
        let id: i64 = stmt.query_row(params![session_id, role.as_str(), content], |row| row.get(0))?;
        Ok(id)
    }
    ...
    pub fn list_messages(&self, session_id: &str, limit: i64) -> Result<Vec<Message>> {
        ...
        while let Some(row) = rows.next()? {
            ...
            out.push(Message{ id, session_id: sid, role: MessageRole::from_str(&role), content, created_at });
        }
        out.reverse();
        Ok(out)
    }
    ...
    pub fn insert_memory_vector(&self, session_id: &str, message_id: Option<i64>, embedding: &[f32]) -> Result<i64> {
        let conn = self.conn()?;
        let embedding_json = serde_json::to_string(embedding)?;
        let mut stmt = conn.prepare("INSERT INTO memory_vectors (session_id, message_id, embedding) VALUES (?, ?, ?) RETURNING id")?;
        let id: i64 = stmt.query_row(params![session_id, message_id, embedding_json], |row| row.get(0))?;
        Ok(id)
    }

    pub fn recall_top_k(&self, session_id: &str, query_embedding: &[f32], k: usize) -> Result<Vec<(MemoryVector, f32)>> {
        let conn = self.conn()?;
        ...
        while let Some(row) = rows.next()? {
            ...
            scored.push((MemoryVector{ id, session_id: sid, message_id, embedding, created_at }, score));
        }
    ...
    pub fn log_tool(&self, tool_name: &str, arguments: &JsonValue, result: &JsonValue, success: bool, error: Option<&str>) -> Result<i64> {
        let conn = self.conn()?;
        let mut stmt = conn.prepare("INSERT INTO tool_log (tool_name, arguments, result, success, error) VALUES (?, ?, ?, ?, ?) RETURNING id")?;
        let id: i64 = stmt.query_row(
            params![tool_name, arguments.to_string(), result.to_string(), success, error.unwrap_or("")],
            |row| row.get(0)
        )?;
        Ok(id)
    }
    ...
    pub fn policy_get(&self, key: &str) -> Result<Option<PolicyEntry>> {
        ...
        if let Some(row) = rows.next()? {
            ...
            Ok(Some(PolicyEntry{ key, value, updated_at }))
        } else { Ok(None) }
    }
}
